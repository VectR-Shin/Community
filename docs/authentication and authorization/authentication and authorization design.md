...
- MANAGER, SUB_MANAGER 권한이 필요한 게시판 관리 기능은 관리 대상 게시판에 대한 관리자 정보와 역할을 조회하여 접근 권한을 검증한다.(RBAC + Resource-based Authorization).
- MANAGER, SUB_MANAGER의 권한은 Community DB에 저장된 게시판별 관리자 정보를 기준으로 판단한다.
