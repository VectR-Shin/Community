...
```
1. 인증된 사용자의 memberId를 확인한다.
2. 회원에 종속된 권한, 제재 및 관계 정보를 삭제한다.
3. 회원 및 프로필 정보를 삭제한다.
4. 인증 제공자의 사용자 정보를 삭제한다.
5. 현재 사용자의 세션을 종료한다.
6. 204 No Content를 반환한다.
```
...
2. 게시글 조회
  - 삭제된 게시판에 속한 게시글은 조회 대상에서 제외한다.
  - 게시글이 존재하지 않거나 조회 대상에서 제외된 경우 오류가 발생한다.
3. 게시글 상세 정보 생성
4. 조회수 처리
  - 원시적 UPDATE 를 사용해 동시성 처리
5. 응답 반환
```

<br>

#### 5.4.4. GET /posts/{postId}/comments?page={pageNumber}
```
Description
- 특정 게시글의 댓글 목록을 조회한다.
...
1. 요청 파라미터 검증
2. 게시글 조회
  - 게시글이 존재하지 않거나 삭제된 게시판에 속한 경우 예외가 발생한다.
3. 해당 게시글의 댓글 목록 조회
4. 댓글 목록 응답 생성
5. 200 OK 반환
```
...
#### 5.4.5. GET /posts/{postId}/comments/popular
```
Description
...
2. 게시글 조회
  - 게시글이 존재하지 않거나 삭제된 게시판에 속한 경우 예외 발생
3. 해당 게시글의 인기 댓글 목록 조회
  - 인기 댓글 정책에 따라 인기 댓글 중 추천 수가 가장 많은 3개를 조회
4. 댓글 목록 응답 생성
5. 200 OK 반환
```
...
3. postId 에 해당하는 게시글 조회
  - 게시글이 존재하지 않거나 게시글이 속한 게시판이 삭제된 경우 예외 발생
4. 요청 사용자의 기존 게시글 반응 조회
  - 요청 사용자의 기존 반응이 있다면 예외 발생
5. 추천 또는 비추천 등록
  - 원시적 UPDATE 로 동시성 제어 필요
6. 201 Created 반환
```
